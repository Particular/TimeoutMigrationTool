namespace Particular.TimeoutMigrationTool.MsmqSql
{
    ...
    public class SqlTTimeoutsTarget : ITimeoutsTarget
    {
        ...
        {
            return DelayedDeliveryTableCreator.DeleteStagingQueue(connection, SqlConstants.TimeoutMigrationStagingTable, schema, connection.Database);
        }
        ...
        async ValueTask EnsureMigrationTableExists()
        {
            try
            {
                await DelayedDeliveryTableCreator.CreateStagingQueue(connection, SqlConstants.TimeoutMigrationStagingTable, schema, connection.Database, preview: false);
            }
            catch (Exception e)
            {
        ...
        public async ValueTask<MigrationCheckResult> AbleToMigrate(EndpointInfo endpoint)
        {
            ...
            try
            {
                await DelayedDeliveryTableCreator.CreateStagingQueue(connection, SqlConstants.TimeoutMigrationStagingTable, schema, databaseName, preview: true);
            }
            catch (Exception e)
            {
            ...
            if (!await DelayedDeliveryTableCreator
