namespace TimeoutMigrationTool.Raven3.AcceptanceTests
{
    ...
    class RavenDBToRabbitMqEndToEnd : RavenDBAcceptanceTest
    {
        ...
        public async Task Can_migrate_timeouts()
        {
            ...
            var context = await Scenario.Define<TargetTestContext>()
                ...
                .When(async (_, c) =>
                {
                    ...
                    var timeoutStorage = new RavenDBTimeoutStorage(logger, serverUrl, databaseName, ravenTimeoutPrefix, ravenVersion);
        ...
        static async Task WaitUntilTheTimeoutIsSavedInRaven(Raven3Adapter ravenAdapter, string endpoint)
        {
            while(true)
            {
