...
namespace TimeoutMigrationTool.Msmq.AcceptanceTests
{
    ...
    public class MigrationRunner
    {
        public static void Run(string connectionString)
        {
            ...
            var args = $@"--fx-version ""{Environment.Version}"" exec {exePath} migrate --allEndpoints sqlp  --source ""{connectionString}"" --dialect MsSqlServer msmq --target ""{connectionString}""";
