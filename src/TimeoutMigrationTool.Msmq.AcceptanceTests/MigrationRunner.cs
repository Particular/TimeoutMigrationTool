...
namespace TimeoutMigrationTool.Msmq.AcceptanceTests
{
    ...
    public class MigrationRunner
    {
        public static void Run(string connectionString)
        {
            ...
            var args = $@"exec {exePath} --fx-version ""{Environment.Version}"" migrate --allEndpoints sqlp  --source ""{connectionString}"" --dialect MsSqlServer msmq --target ""{connectionString}""";
